...
Статус: реализован базовый запуск через `ProcessRunner`.
...
- `CodexCommandBuilder`
  ...
  - если в `PromptExecutionRequest` есть `codexSessionId`, строит команду
    `codex exec resume <SESSION_ID> -` для продолжения Codex context проекта;
  - не использует shell escaping.
- `CodexOutputParser`
  - выделяет полезный stdout;
  - выделяет error message;
  - извлекает `thread_id` из JSONL и возвращает его как
...
Контекстная модель: один `Project` хранит один optional `codexSessionId`. Первый
Codex-запуск проекта создаёт session, parser извлекает `thread_id`, а
application layer сохраняет его в проект. Последующие prompt-ы этого project
используют `codex exec resume`.
