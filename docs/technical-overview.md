...
`COMPLETED` означает, что на момент последнего запуска в очереди не осталось
`QUEUED` prompt-ов. Если позже в такую очередь добавили новые prompt-ы, runner
может переоткрыть очередь и выполнить их обычным способом.

### Prompt

Промпт относится к очереди и целевому AI-инструменту. Он содержит title,
content, queue position, priority, максимальное число попыток и опциональную
рабочую директорию.
...
2. Меньшая queue position выполняется раньше.
3. При равной position используется более высокий priority.
4. При равной position и priority используется более ранний `createdAt`.
...
4. Если очередь была `COMPLETED`, но появился новый queued prompt, переоткрыть
   её для нового запуска.
5. Проверить доступность целевого AI-инструмента.
6. При необходимости ожидать лимит или подтверждение пользователя.
7. Создать execution и перевести prompt в `RUNNING`.
8. Выполнить prompt через подходящий adapter.
9. Сохранить результат и обновить состояния prompt и queue.
10. Остановить очередь при ошибке, если включён `stopOnError`.
11. Отправить уведомления и продолжить согласно execution policy.
...
4. Использовать queue position и priority ordering.
...
- retry attempts;
- queue position и priority ordering;
